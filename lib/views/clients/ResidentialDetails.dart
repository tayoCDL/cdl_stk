...
class ResidentialDetails extends StatefulWidget {
  ...
  final int? ClientInt,
      ResidentialPermanentResidentialState,
      ResidentialPermanentLGA,
      ResidentialStatus,
      ResidentialNoOfYears;
  final String? permanentAddress, nearestLandmark, ComingFrom;
  const ResidentialDetails(
      {Key? key,
      this.ResidentialPermanentResidentialState,
      this.ResidentialPermanentLGA,
      this.ResidentialStatus,
      this.ResidentialNoOfYears,
...
class _ResidentialDetailsState extends State<ResidentialDetails> {
  ...
  void initState() {
    ...
    year_at_residence = ResidentialNoOfYears == null ? '' : ResidentialNoOfYears.toString();
  ...
  getResidentialInformation() async {
    ...
    prefs.setInt('tempResidentialInt',
        residentialProfile.isEmpty ? null : residentialProfile[0]['addressId']);
    ...
    dateMovedIn.text = retDOBfromBVN(
        '${residentialProfile[0]['dateMovedIn'][0]}-${residentialProfile[0]['dateMovedIn'][1]}-${residentialProfile[0]['dateMovedIn'][2]}');
    ...
    var saveResidential = () async {
      ...
      postAndPut.isClientActive(localclientID).then((value) {
        ...
        Map<String, dynamic> residentialData = {
          ...
          'address_type_id': 36,
          'permanent_address':
              permanent_address.text == 0 ? 'N/A' : permanent_address.text,
          "nearest_landmark":
              nearest_landmark.text == 0 ? 'N/A' : nearest_landmark.text,
          'residential_status': residentialInt,
          "year_at_residence": year_at_residence,
          'dateMovedIn': dateMovedIn.text
        };
        final Future<Map<String, dynamic>> respose =
            addClientProvider.addResidential(residentialData, client_status);
        ...
        respose.then((response) {
          AppTracker().trackActivity('ADD/UPDATE RESIDENTIAL',
              payLoad: {...residentialData, "response": response.toString()});
                            ...
                            children: [
                              ...
                              Padding(
                                padding: EdgeInsets.symmetric(
                                    horizontal: 20, vertical: 10),
                                child: DropDownComponent(
                                    items: residentialArray,
                                    onChange: (String? item) async {
                                      setState(() {
                                        List<dynamic> selectID = allResidential
                                            .where((element) =>
                                                element['name'] == item)
                                            .toList();
                                        print('this is select ID');
                        ...
                        child: Container(
                          ...
                          child: Padding(
                            ...
                            child: Container(
                              ...
                              child: TextFormField(
                                style:
                                    TextStyle(fontFamily: 'Nunito SansRegular'),
                                autofocus: false,
                                readOnly: true,
              ...
              children: [
                Container(
                  ...
                  child: CupertinoDatePicker(
                    ...
                    minimumDate:DateTime.now().subtract(Duration(days: 18250)) ,
                  ),
                ),
                CupertinoButton(
                  child: const Text('OK'),
