...
class CreateWacsLoanTerms extends StatefulWidget {
  ...
  @override
  _CreateWacsLoanTermsState createState() => _CreateWacsLoanTermsState(
...
class _CreateWacsLoanTermsState extends State<CreateWacsLoanTerms> {
  ...
  //  int productID = otherInfo['loanProductId'];
    ...
    respose.then((response) {
      ...
      setState(() {
      //  Map<String,dynamic> mapDynamicData = thirdPartyLoanResponse['staff_info']['data'];
       // netpay.text = mapDynamicData['net_pay'].toString();

        fullTemps = fullTemplate;

        print('product other config ${fullTemps['product']['otherConfig']}');
        print('interest rate per period ${fullTemps['product']['interestRatePerPeriod'].toString()}');
...
// Employer charges should be checked
        ...
        comp_max_principal = (fullTemps['employerLoanProductDataOptions'] == null
            ? fullTemps['product']['maxPrincipal']
            : fullTemps['employerLoanProductDataOptions'][0]
        ['maxPrincipal'] ??
            fullTemps['product']['maxPrincipal']).toString();
...
// SANDBOX
  ...
  confirmRequestAndBookLoan() async{
    ...
    final Future<Map<String,dynamic>> response =   RetCodes().post_encryptAndSend(sendLoanDataForThirdParty,tp_customerId: otherInfo['id'],channelId: 2);
    setState(() {
      _isLoading = true;
    });
    response.then((responseData) {
      ...
      setState(() {
        ...
        if(responseData['statusCode'] == 200){
          ...
          Flushbar(
            ...
            if(responseData['message'].contains('unique')){
              MyRouter.pushPageReplacement(context, EmbeddedNewLoan(clientID: clientID,));
              Flushbar(
                flushbarPosition: FlushbarPosition.TOP,
                flushbarStyle: FlushbarStyle.GROUNDED,
                backgroundColor: Colors.red,
                title: "Error",
                message: 'We are currently experiencing an error booking your loan,kindly restart this loan process again',
                duration: Duration(seconds: 6),
              ).show(context);
            }
            else {
  ...
  geSingleLoanConfig(int configID) async{
    ...
    respose.then((response) async {
      ...
      //   MyRouter.pushPagge(context, DocumentExtraScreen(
      //     // clientID: clientID,
      //   ));
      // }
      ...
      //   print('second documents required are ${collectDocumentType}');
      //   // MyRouter.pushPage(
      //   //     context,
      //   //     DocumentForLoan(
      //   //       clientID: clientID,
                  ...
                  Column(
                    children: [
                      ...
                      Padding(
                        ...
                        child: Align(
                            alignment: Alignment.topLeft,
                            child: Text('Principal (min: ₦${min_principal}, max: ₦${max_principal})*',
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Align(
                                  alignment: Alignment.topLeft,
                                  child: Text('Loan Tenure (min: ${min_repayment} months, max: ${max_repayment} months)*',
                                    style: TextStyle(fontSize: 12,fontWeight: FontWeight.w300,fontFamily: 'Nunito SemiBold'),
                                    textAlign: TextAlign.start,)),
                            ),
                          ],
          ...
          callAction2: () {
            ...
            if(netpay.text.isEmpty){
              Flushbar(
                ...
                backgroundColor: Colors.red,
                title: "Error",
                message: 'Netpay cannot be empty',
                duration: Duration(seconds: 6),
              ).show(context);
              ...
              Flushbar(
                ...
                message: 'Your netpay is outside the allowable netpay',
                duration: Duration(seconds: 6),
              ).show(context);
              ...
              Flushbar(
                ...
                backgroundColor: Colors.red,
                title: "Validation Error",
                message: 'You selected an amount, outside of the required amount allowed for this product',
                duration: Duration(seconds: 6),
              ).show(context);
            ...
            backgroundColor: Colors.redAccent,
            title: 'Validation Error',
            message: 'You selected a tenor outside of the required tenor allowed for this product ',
            duration: Duration(seconds: 3),
            ).show(context);
            }
        ...
        children: [
          Column(
            ...
            children: [
              ...
              SizedBox(height: 5,),
              SizedBox(height: 2,),
              Text('Eligibility Amount',style: TextStyle(color: ColorUtils.COLOR_TEXT,fontFamily: 'Nunito SansRegular',fontSize: 12),),
              SizedBox(height: 5,),
              Text(otherInfo['eligibility'] == null ? 'N/A' : '₦${AppHelper().formatCurrency(otherInfo['eligibility'].toString())}'?? '',style: TextStyle(color: ColorUtils.WHITE_COLOR,fontFamily: 'Nunito SemiBold',fontSize: 14),),
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  SizedBox(height: 22,),
                  Text('Monthly Eligibility',style: TextStyle(color: ColorUtils.COLOR_TEXT,fontFamily: 'Nunito SansRegular',fontSize: 12),),
                  SizedBox(height: 5,),
              //    Text(otherInfo['monthlyEligibility'] ?? '',style: TextStyle(color: ColorUtils.WHITE_COLOR,fontFamily: 'Nunito SemiBold',fontSize: 14),),
                  Text(otherInfo['monthlyEligibility'] == null ? 'N/A' : '₦${AppHelper().formatCurrency(otherInfo['monthlyEligibility'].toString())}'?? '',style: TextStyle(color: ColorUtils.WHITE_COLOR,fontFamily: 'Nunito SemiBold',fontSize: 14),),
